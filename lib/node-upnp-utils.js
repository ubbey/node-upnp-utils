...
* 
* Whenever a root device is newly found, this method emits 'added'
...
UPnPUtils.prototype._prepareMulticastUdpSocket = function (callback) {
	...
	sock.on("error", (err) => {
		console.log("sock err in multi")
		sock.close(() => {
			console.log("sock close in multi")
			this.emit('error', err);
		});
	});
...
UPnPUtils.prototype._prepareUnicastUdpSocket = function (callback) {
	...
	sock.on("error", (err) => {
		console.log("sock err in unicast")

		sock.close(() => {
			console.log("sock close in unicast")
...
UPnPUtils.prototype._udpSocketListener = function (buffer, rinfo) {
	...
	var boxId = loc.replace(/\?id=(\w+)$/g, "$1");
	usn = boxId;
	if (!usn) {
		if (!this.devices[usn]) {
			this.devices[usn] = {};
...
UPnPUtils.prototype.stopDiscovery = function (callback) {
	...
	if (this.discovery_started === true) {
		...
		//发送multicast数据，否则不能触发close的回调
		this.sockets['multicast'].send("close", 0, 5, this.MULTICAST_PORT, this.MULTICAST_ADDR);
		console.log("关闭socket连接......");
		this.sockets['multicast'].close(() => {
			console.log("multicast close成功！！")
